...
const Errors = require("../api/errors/uvod-main-error.js");
...
const logger = LoggerFactory.get("UvodMainAbl");

class UvodMainAbl {
  constructor() {
    this.validator = Validator.load();
    this.dao = DaoFactory.getDao(Schemas.UVOD_MAIN);
  }
  ...
  async init(uri, dtoIn, session) {
    ...
    let uvodMain;
    try {
      uvodMain = await this.dao.create(uuObject);
    } catch (e) {
      throw new Errors.Init.UvodDaoCreateFailed({ uuAppErrorMap }, e);
    }
...
module.exports = new UvodMainAbl();
